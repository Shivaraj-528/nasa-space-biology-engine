...
import { datasetDetailService } from '../services/datasetDetailService';
...
router.get('/datasets/:id', authenticate, async (req, res) => {
  try {
    ...
    const detailedDataset = await datasetDetailService.getDetailedDataset(id);
...
router.get('/datasets/:id/files', authenticate, async (req, res) => {
  try {
    const { id } = req.params;
...
router.get('/datasets/:id/files/:filename', authenticate, async (req, res) => {
  try {
...
router.get('/datasets/:id/publication', authenticate, async (req, res) => {
  try {
    const { id } = req.params;
...
router.get('/datasets/:id/related', authenticate, async (req, res) => {
  try {
    const { id } = req.params;
...
router.get('/datasets/:id/citation', authenticate, async (req, res) => {
  try {
    const { id } = req.params;
...
router.post('/datasets/:id/download-request', authenticate, async (req, res) => {
  try {
    const { id } = req.params;
...
// Helper functions
function generateSampleFileContent(file: any): string {
...
function generateSampleCSV(file: any): string {
...
function generateSampleText(file: any): string {
...
function generateSampleJSON(file: any): string {
...
function generateCitation(dataset: any, format: string): string {
...
function generateBibtex(dataset: any): string {
  const year = new Date(dataset.createdAt).getFullYear();
...
function generateEndnote(dataset: any): string {
  const year = new Date(dataset.createdAt).getFullYear();
