...
exports.nasaChatbot = exports.NASAChatbotService = void 0;
const axios_1 = __importDefault(require("axios"));
const Dataset_1 = require("../models/Dataset");
// In-memory storage for demo (in production, use Redis or database)
const chatSessions = new Map();
class NASAChatbotService {
    constructor() {
        this.nasaKnowledgeBase = [];
    ...
    async initializeKnowledgeBase() {
        try {
            ...
            const datasets = await Dataset_1.Dataset.find({}).limit(100);
    ...
    findRelevantDatasets(query, limit = 5) {
        ...
        const scored = this.nasaKnowledgeBase.map(dataset => {
            ...
            if (dataset.description && dataset.description.toLowerCase().includes(queryLower))
                score += 5;
            // Organism matches
            if (dataset.organism && queryLower.includes(dataset.organism.toLowerCase()))
                score += 8;
            // Type matches
            if (dataset.type && queryLower.includes(dataset.type))
                score += 6;
            // Source matches
            if (dataset.source && queryLower.includes(dataset.source.toLowerCase()))
                score += 4;
            // Keyword matches
            keywords.forEach(keyword => {
                if (dataset.searchableText.includes(keyword))
                    score += 2;
            });
    ...
    async callGrokAPI(messages, context) {
        if (!this.grokApiKey) {
...
- Be helpful with both scientific and general queries`
    ...
    async callOpenAIAPI(messages, context) {
...
- Be helpful and informative`
    ...
    generateLocalResponse(query, relevantDatasets) {
        const queryLower = query.toLowerCase();
...
What specific aspect of space biology interests you most?`;
        ...
        const datasetInfo = relevantDatasets.slice(0, 3).map(dataset => `• **${dataset.title}** (${dataset.source})\n  🔬 Organism: ${dataset.organism || 'Various'}\n  📊 Type: ${dataset.type}\n  ${dataset.description ? '📝 ' + dataset.description.substring(0, 120) + '...' : ''}`).join('\n\n');
...
What aspect interests you most?`;
    ...
    generateQuerySuggestions(queryLower) {
...
• "What are the health effects of spaceflight?"`;
    ...
    async processQuery(sessionId, userId, query) {
        try {
            ...
            const context = relevantDatasets.map(d => `Dataset: ${d.title}\nOrganism: ${d.organism}\nDescription: ${d.description}`).join('\n\n');
            let responseContent;
            let confidence = 0.7;
            try {
...
exports.NASAChatbotService = NASAChatbotService;
exports.nasaChatbot = new NASAChatbotService();
