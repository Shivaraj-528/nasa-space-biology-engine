...
exports.datasetDetailService = exports.DatasetDetailService = void 0;
const Dataset_1 = require("../models/Dataset");
class DatasetDetailService {
    async getDetailedDataset(datasetId) {
        try {
            // First try to get the basic dataset
            const dataset = await Dataset_1.Dataset.findById(datasetId);
            if (!dataset) {
    ...
    async enhanceDatasetWithDetails(dataset) {
    ...
    generateExperimentalDetails(dataset) {
    ...
    async generatePublicationInfo(dataset) {
    ...
    generateDatasetFiles(dataset) {
        ...
        const templates = fileTemplates[dataset.type] || fileTemplates.genomics;
    ...
    generateAnalysisInfo(dataset) {
        ...
        const datasetSoftware = software[dataset.type] || software.genomics;
        const datasetMethods = methods[dataset.type] || methods.genomics;
        return {
    ...
    async findRelatedStudies(dataset) {
        try {
            // Find related datasets from the database
            const relatedDatasets = await Dataset_1.Dataset.find({
                _id: { $ne: dataset._id },
                $or: [
                    { organism: dataset.organism },
                    { type: dataset.type },
            ...
            const relatedStudies = relatedDatasets.map(related => ({
    ...
    getPlatformForType(type) {
        ...
        const typeOptions = platforms[type] || platforms.genomics;
        return typeOptions[Math.floor(Math.random() * typeOptions.length)];
    }
    getTissueForOrganism(organism) {
        if (!organism)
    ...
    getGrowthConditions(organism) {
        if (!organism)
            return 'Standard laboratory conditions';
        if (organism.toLowerCase().includes('arabidopsis')) {
    ...
    getMeasurementTechniques(type) {
        ...
        return techniques[type] || techniques.genomics;
    }
    generateCoInvestigators(allPIs, currentPI) {
        return allPIs.filter(pi => pi !== currentPI).slice(0, Math.floor(Math.random() * 3) + 1);
    }
    getDepartmentForInstitution(institution) {
...
Conclusions: These findings provide new insights into the molecular mechanisms underlying ${dataset.organism || 'biological'} adaptation to space environments and have implications for astronaut health and space agriculture.`;
    ...
    calculateSimilarity(dataset1, dataset2) {
        let similarity = 0;
        if (dataset1.organism === dataset2.organism)
            similarity += 0.4;
        if (dataset1.type === dataset2.type)
            similarity += 0.3;
        if (dataset1.source === dataset2.source)
            similarity += 0.2;
        if (dataset1.assay_type === dataset2.assay_type)
            similarity += 0.1;
        return Math.min(similarity + Math.random() * 0.1, 1.0);
    }
    determineRelationshipType(dataset1, dataset2) {
        if (dataset1.organism === dataset2.organism)
    ...
    generateSyntheticRelatedStudies(dataset) {
        const templates = [
...
exports.DatasetDetailService = DatasetDetailService;
exports.datasetDetailService = new DatasetDetailService();
