...
const oauthState_1 = require("../services/oauthState");
...
const base64url = (input) => Buffer.from(input)
    ...
    .replace(/\+/g, '-')
    .replace(/\//g, '_');
const createRandomString = (bytes = 32) => base64url(crypto_1.default.randomBytes(bytes));
...
router.post('/auth/login', async (req, res) => {
    ...
    const token = (0, auth_1.signToken)({ sub: user.id, name: user.name, email: user.email, role });
    return res.json({ user, token });
});
// GET /api/v1/auth/me
router.get('/auth/me', auth_1.authenticate, (req, res) => {
...
router.get('/auth/start', (_req, res) => {
    ...
    void (0, oauthState_1.storeState)(state, code_verifier, Math.floor(STATE_TTL_MS / 1000));
...
router.get('/auth/callback', async (req, res) => {
    ...
    const verifier = await (0, oauthState_1.getAndDeleteState)(state);
    if (!verifier)
    ...
    try {
        ...
        const tokenRes = await axios_1.default.post(OAUTH2_TOKEN_URL, querystring_1.default.stringify(body), { headers: { 'Content-Type': 'application/x-www-form-urlencoded' } });
        const { access_token /*, id_token, token_type, expires_in */ } = tokenRes.data || {};
        if (!access_token) {
            return res.status(502).json({ error: 'Token exchange failed' });
        }
        // Fetch userinfo
        const userRes = await axios_1.default.get(OAUTH2_USERINFO_URL, {
            headers: { Authorization: `Bearer ${access_token}` },
        });
        const profile = userRes.data || {};
        const email = profile.email || profile.preferred_username || 'unknown@nasa.gov';
        ...
        const role = ADMIN_EMAILS.includes(lower)
            ...
            : (lower.endsWith('@nasa.gov') ? 'nasa_scientist' : 'researcher');
        const appUser = {
            id: 'oauth-' + crypto_1.default.createHash('sha1').update(email).digest('hex').slice(0, 12),
            name,
            email,
            role,
        };
        const appToken = (0, auth_1.signToken)({ sub: appUser.id, name: appUser.name, email: appUser.email, role });
