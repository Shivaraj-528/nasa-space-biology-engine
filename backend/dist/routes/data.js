...
router.get('/datasets', async (req, res) => {
    try {
        ...
        let total = 0;
        try {
            total = await Dataset_1.Dataset.countDocuments(filter);
            items = await Dataset_1.Dataset.find(filter)
                .sort({ createdAt: -1 })
                .skip((page - 1) * limit)
                .limit(limit)
                .lean();
...
router.post('/datasets', async (req, res) => {
    try {
        ...
        const doc = await Dataset_1.Dataset.create({ source, type, title });
...
router.get('/biological/:organism/experiments', async (req, res) => {
    try {
        ...
        let total = 0;
        try {
            total = await sql_1.Experiment.count({ where: { organism } });
            items = await sql_1.Experiment.findAll({ where: { organism }, offset: (page - 1) * limit, limit, order: [['created_at', 'DESC']] });
        }
        catch (dbErr) {
            // Fallback mock
            items = [
