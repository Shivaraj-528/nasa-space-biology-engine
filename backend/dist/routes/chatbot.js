...
router.post('/chatbot/query', auth_1.authenticate, async (req, res) => {
    try {
        ...
        const response = await chatbotService_1.nasaChatbot.processQuery(finalSessionId, userId, message);
        res.json({
...
router.get('/chatbot/session/:sessionId', auth_1.authenticate, async (req, res) => {
    try {
        const { sessionId } = req.params;
        const userId = req.user?.sub || 'anonymous';
        const session = chatbotService_1.nasaChatbot.getSession(sessionId);
        if (!session) {
...
router.get('/chatbot/sessions', auth_1.authenticate, async (req, res) => {
    try {
        const userId = req.user?.sub || 'anonymous';
        const sessions = chatbotService_1.nasaChatbot.getSessions(userId);
...
router.delete('/chatbot/session/:sessionId', auth_1.authenticate, async (req, res) => {
    try {
        const { sessionId } = req.params;
        const userId = req.user?.sub || 'anonymous';
        const session = chatbotService_1.nasaChatbot.getSession(sessionId);
        ...
        const deleted = chatbotService_1.nasaChatbot.clearSession(sessionId);
        if (deleted) {
